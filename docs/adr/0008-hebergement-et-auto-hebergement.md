...
`jadwal` permet à toute organisation de publier le programme de ses cours récurrents à partir d'une
seule saisie. La V1 est gratuite ; des offres payantes viendront plus tard. Aucun paiement n'est
codé en V1.
...
prévu à l'étape 8 : durcissement, déploiement, pose sur le site de la première organisation.

## Décision

- Le même code sert aux deux modes de déploiement.
- Service hébergé : l'auteur exploite une instance officielle, gratuite en V1 ; l'accès payant
  viendra plus tard.
...
- Le durcissement, le déploiement de l'instance officielle et la pose sur le site de la première
  organisation sont reportés à l'étape 8 ; le VPS dédié reste à provisionner à cette étape.
...
déploiement, pose sur le site de la première organisation).
